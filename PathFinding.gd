	...
	for tile in tiles:
		...
		for x in range(-1,2):
			for y in range(-1,2):
				...
				if tile == target or not astar.has_point(targetId): #or x*x+y*y>1:
					continue
			...
			for x in range(-1,2):
				for y in range(-1,2):
					...
					if astar.has_point(id):#calculate distance between obstacle and tile
						...
						if worldPoint.distance_to(obstacle.global_position)<=sqrt(2)*half_cell_size.x:#the closer the obstacle is to the tile-the larger the weight
							astar.set_point_weight_scale(id,2*sqrt(2)*half_cell_size.x/worldPoint.distance_to(obstacle.global_position))
